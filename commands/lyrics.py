        ...
        async with ctx.channel.typing():
            try:
                network = pylast.LastFMNetwork(
                    api_key=self.bot.LASTFM_API_KEY,
                    api_secret=self.bot.LASTFM_API_SECRET,
                )
            ...
            if method == "full":
                try:
                    ...
                    artisturl = "https://www.last.fm/de/music/" + str(
                        lied.get_artist()
                    ).replace(" ", "+")
                    songurl = (
                        artisturl
                    ...
                    if user.get_image() is not None:
                        ...
                        embed.set_author(
                            name=username, url="https://www.last.fm/user/" + username
                        )
                    ...
                    footer = (
                        ...
                        + " | "
                        + "Plays: "
                        + str(lied.get_playcount())
                    )
                    embed.set_footer(text=footer)
                    try:
                        genius = lyricsgenius.Genius(self.bot.LYRICS_KEY)
                        text = genius.search_song(
                            title=str(lied.get_name()), artist=str(lied.get_artist())
                        )
                        gesamter_text = str(text.lyrics).replace(
                            "EmbedShare URLCopyEmbedCopy", ""
                        )[0:5500]
                        while gesamter_text != "":
                            embed.add_field(
                                name="Fortsetzung",
                                value=(gesamter_text[0:1020]),
                                inline=False,
                            )
                ...
                try:
                    ...
                    song = genius.search_song(
                        title=lied.get_title(), artist=lied.get_artist()
                    )
                    embed.add_field(
                        name="Link",
                        value=str(
