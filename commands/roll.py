        ...
        try:
            if nummern:
                if len(nummern) > 2:
                    ...
                    logger.info("roll(): Zu viele Argumente: " +
                                ctx.author.name)
                elif len(nummern) == 1:
                    zahl = int(nummern[0])
                    if isinstance(zahl, int):
                        if zahl > 0:
                            ...
                            logger.info(
                                "roll(): Zahl zu klein von: " + ctx.author.name)
                    else:
                        await ctx.reply("Das is keine Zahl, du Monger.")
                        logger.info("roll(): Keine Zahl von: " +
                                    ctx.author.name)
                else:
                    zahl1 = int(nummern[0])
                    zahl2 = int(nummern[1])
                    if isinstance(zahl1, int) and isinstance(zahl2, int):
                        if zahl1 > 0 and zahl2 > 0:
                            if zahl1 > 20:
                                await ctx.reply("Pls nicht mehr als 10 Würfe auf einmal.")
                                logger.info(
                                ...
                                embed = discord.Embed(
                                    title="Würfelwurf", description="Veni Vidi Ficki", color=0x00ff00)
                                embed.set_author(
                                    name="Gott", icon_url="https://upload.wikimedia.org/wikipedia/commons/7/7c/Cima_da_Conegliano%2C_God_the_Father.jpg")
                ...
                logger.info(
                    "roll(): Keine Frage gestellt von: " + ctx.author.name)
