...
class RemindMe(commands.Cog):
    ...
    async def remindme(self, ctx, method: str, *text: str):
        ...
        self.db.commit()
        self.cursor.execute(
            "SELECT id FROM reminders WHERE reminder_time=?", (reminder_time,)
        )
        ...
        await self.wait_for_reminder(
            ctx.author.id, reason, reminder_time, message, channel, id
        )

    async def wait_for_reminder(
        self, user_id, reminder_text, reminder_time1, message, channel, id
    ):
        try:
            if (reminder_time1 - time.time()) < 0:
                try:
                    await message.reply(
                        "Ich werde dich wissen lassen:\n**{}**".format(reminder_text),
                        mention_author=True,
                    )
    ...
    async def get_reminder_startup(self):
        try:
            ...
            if result:
                ...
                await self.wait_for_reminder_startup(
                    id, reminder_text, reminder_time1, channel_id, message
                )
        except Exception as e:
            await message.channel.send(
    ...
    async def wait_for_reminder_startup(
        self, id, reminder_text, reminder_time1, channel_id, message
    ):
        try:
            channel = self.bot.get_channel(channel_id)
            if (reminder_time1 - time.time()) < 0:
